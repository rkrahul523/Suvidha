...
export class Calendar {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      console.log(
        `Rendering: ${month + 1}/${year} - Leaves found:`,
        this.weeks()
          .flat()
      ...
      for (let dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
        ...
        if (date >= 1 && date <= daysInMonth) {
          const dayLeaves = this.leavesConfig().filter((leave) => {
            ...
            return fromYear === year && fromMonth === month && fromDay <= date && toDay >= date;
          });
  ...
  selectDay(day: CalendarDay) {
    if (!day.isEmpty) {
      console.log(
        'Selected:',
        `${day.date}-${this.currentMonthIndex() + 1}-${this.currentYear()}`,
        day.leaves,
      );
    }
  }
  ...
  currentMonth(): string {
    return new Date(this.currentYear(), this.currentMonthIndex(), 1).toLocaleDateString('en-US', {
      month: 'long',
    });
  }
